...
class RefreshScreen {
    ...
    static void refreshScreen(Player player, GenerateMaze board){
        for(int i = 0; i < board.getBoardWidth(); i++){ //going through the array that contains the arrays
            for(int j = 0; j < board.getBoardHeight(); j++){ //going through the array in the array
                //System.out.println("i: " +i + " j: " + j);
                //System.out.println(board.getBoardHeight() + "   " + board.getBoardWidth());
                //if the player is not in the correct position then delete them from that position,
                //if the loop reaches the position the player should be at, then place the player there
